...
if st.session_state['user_id'] is None:
    ...
    with col2:
        st.markdown("<h2 style='text-align:right;'>سیبتک 🍎 مدیریت نهال</h2>", unsafe_allow_html=True)

    mode = st.radio("حالت:", ["ورود", "ثبت‌نام", "دمو"], horizontal=True)
    ...
    # ---------- Prediction ----------
        ...
        if f:
            ...
            if model:
                ...
                result = list(disease_info.keys())[class_idx] if disease_info else "سالم"
                confidence = f"{preds[0][class_idx]*100:.2f}%"
                st.success(f"نتیجه: {result} ({confidence})")
                info = disease_info.get(result, {})
                st.write("**نوع بیماری:**", info.get("type","–"))
                st.write("**روش درمان:**", info.get("treatment","–"))
                st.write("**اطلاعات جانبی:**", info.get("notes","–"))
                st.write("**شدت/میزان:**", info.get("severity","–"))
                # Save prediction
                try:
                    ...
                    st.error("خطا در ذخیره پیش‌بینی:", e)
            else:
                st.warning("مدل پیش‌بینی در دسترس نیست.")
